...
struct Config {
    ...
    var position: String? = nil  // "top-left", "top-right", "bottom-left", "bottom-right", "center"
}
...
func parseArgs() -> Config {
    ...
    while i < args.count {
        ...
        case "--browser":
            i += 1
            if i < args.count { config.browser = args[i] }
        case "--position":
            i += 1
            if i < args.count { config.position = args[i] }
        default:
            break
        }
        i += 1
...
class AppDelegate: NSObject, NSApplicationDelegate, WKNavigationDelegate, WKScriptMessageHandler, NSWindowDelegate {
    ...
    private func setupWindow() {
        ...
        } else if let position = config.position {
            // Named positions relative to the visible screen area
            let visible = NSScreen.main?.visibleFrame ?? NSRect(x: 0, y: 0, width: 1512, height: 900)
            let wSize = window.frame.size
            switch position {
            case "top-left":
                window.setFrameOrigin(NSPoint(x: visible.origin.x, y: visible.origin.y + visible.height - wSize.height))
            case "top-right":
                window.setFrameOrigin(NSPoint(x: visible.origin.x + visible.width - wSize.width, y: visible.origin.y + visible.height - wSize.height))
            case "bottom-left":
                window.setFrameOrigin(NSPoint(x: visible.origin.x, y: visible.origin.y))
            case "bottom-right":
                window.setFrameOrigin(NSPoint(x: visible.origin.x + visible.width - wSize.width, y: visible.origin.y))
            default:
                window.center()
            }
    ...
    nonisolated func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        MainActor.assumeIsolated {
            // Allow initial page loads and internal navigations (about:blank, data:, loadHTMLString)
            ...
            // Allow about: and data: URLs (internal WKWebView content)
            if scheme == "about" || scheme == "data" || scheme.isEmpty {
                decisionHandler(.allow)
                return
            }
            ...
            if let browserApp = config.browser {
                // Open in the specified browser application
                let browserURL = URL(fileURLWithPath: browserApp)
